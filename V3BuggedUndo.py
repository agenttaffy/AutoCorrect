...
"""
╔══════════════════════════════════════════════════════╗
║        AutoCorrect Daemon  v3.0 (No Fuzzy)          ║
...
#  Add tons of valid words here or load them from files.
# ─────────────────────────────────────────────────────
BASE_WORDS = """
...
_ignore_corrections_until = 0.0
_ignore_exact_word = None
...
def build_vocabulary():
    ...
    # Base built-in word list
    add_words_from_iterable(BASE_WORDS.split(), weight=20)

    # Explicit correction targets get boosted so common intended words win
    ...
    # Optional corpus for probability ranking
...
def candidate_score(original: str, candidate: str):
    ...
    2. Smaller length difference
    3. Slight preference for candidates sharing first letter
    """
...
def choose_best_candidate(original: str, candidates):
    ...
    for cand in candidates:
        ...
        # Never collapse a possible inflected form to a stem
        if looks_like_simple_inflection(original, cand):
            continue

        # Avoid wild length jumps
    ...
    return max(filtered, key=lambda c: candidate_score(original, c))

def correction(word: str) -> str | None:
    global _ignore_corrections_until, _ignore_exact_word

    lower = word.lower()

    # If we just undid this exact word, leave it completely alone briefly
    if (
        _ignore_exact_word == lower
        and time.time() < _ignore_corrections_until
    ):
        return None
    ...
    2. If known valid word -> leave unchanged
    3. Known 1-edit candidates (with confidence gate)
    4. Known 2-edit candidates only for longer words
    """
    ...
    # 1. Explicit typo mapping always wins
    if lower in CORRECTIONS:
        return preserve_case(word, CORRECTIONS[lower])

    # 2. Known valid word? leave it alone
    if lower in WORDS:
        return None

    # 3. Don't guess on short words
    if len(lower) <= 4:
        return None
    ...
    def maybe_pick(candidates):
        ...
        # First and last letters should match
        ...
        # Best should clearly beat runner-up
        if second is not None and p_best < (1.5 * p_second):
            return None

        # Extra caution for mid-length words: avoid wild guesses
    ...
    # 5. Two-edit candidates only for longer words
    if len(lower) >= 7:
...
def _apply_correction(original: str, corrected: str, boundary_was_space: bool):
    global _suppress_typed_keys, _last_correction, _word_buffer
    ...
    try:
        ...
        replacement_text = corrected + (" " if boundary_was_space else "")
        restore_text = original + (" " if boundary_was_space else "")
...
def _undo_last_correction():
    global _last_correction, _suppress_typed_keys
    global _ignore_corrections_until, _ignore_exact_word, _word_buffer

    if not _last_correction:
        return

    replacement_text = _last_correction["replacement_text"]
    restore_text = _last_correction["restore_text"]

    with _suppress_lock:
        _suppress_typed_keys = True

    try:
        delete_count = len(_last_correction["replacement_text"])
        ...
        controller.type(restore_text)

        _word_buffer.clear()

        _ignore_exact_word = _last_correction["original"].lower()
        _ignore_corrections_until = time.time() + 1.5

        print(f"  [UNDO]  {_last_correction['corrected']}  →  {_last_correction['original']}")
        _last_correction = None
...
def _flush_buffer(boundary_was_space: bool):
    ...
    # Safety: if buffer somehow has spaces, bail (don't try to correct multi-word)
    if " " in stripped:
        return
...
# ─────────────────────────────────────────────────────
def on_press(key):
    global _CTRL_DOWN, _SHIFT_DOWN, _enabled, _last_backspace_time, _last_correction, _backspace_down
    ...
    try:
        if key in BOUNDARY_KEYS:
            ...
            # Ignore OS auto-repeat while key is held
            if _backspace_down:
                return
            ...
            if (
                now - _last_backspace_time <= UNDO_WINDOW
                and _last_correction is not None
                and now - _last_correction["time"] <= UNDO_MAX_AGE
            ):
                _undo_last_correction()
                _word_buffer.clear()
                _last_backspace_time = 0.0
                return
